...
func (t TopologyDomainGroup) ForEachDomain(pod *v1.Pod, filter TopologyNodeFilter, f func(domain string)) {
	domainDebugLog("ForEachDomain called for pod %s/%s, total domains in group: %d", pod.Namespace, pod.Name, len(t))
	...
	if len(excludedDomains) > 0 {
		domainDebugLog("  WARNING: Some domains were excluded! This may cause TSC issues with volumes.")
	}
}
...
func (d *topologyDomainAttributes) matches(filter TopologyNodeFilter, pod *v1.Pod) bool {
	if filter.TaintPolicy == v1.NodeInclusionPolicyHonor {
		...
		if !taintMatched {
			domainDebugLog("    Domain rejected: taints not tolerated")
			return false
		}
	}
